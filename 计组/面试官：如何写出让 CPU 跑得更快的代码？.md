...
![img](https://gitee.com/studylihai/pic-repository/raw/master/%5Cimg/20201119123107.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/%5Cimg/20201119124148.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/%5Cimg/20201119124153.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/%5Cimg/20201119124157.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/%5Cimg/20201119124200.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/%5Cimg/20201119124204.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/%5Cimg/20201119124213.png)
...
因此，一个内存的访问地址，包括==**组标记(Tag)、CPU Line 索引(Index)、偏移量(Offset)**==这三种信息，于是 CPU 就能通过这些信息，在 CPU Cache 中找到缓存的数据。而对于 CPU Cache 里的数据结构，则是由**索引 + 有效位 + 组标记 + 数据块**组成。

![img](https://gitee.com/studylihai/pic-repository/raw/master/%5Cimg/20201119124218.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/%5Cimg/20201119124223.png)

经过测试，形式一 `array[i][j]` 执行时间比形式二 `array[j][i]` 快好几倍。

之所以有这么大的差距，是因为二维数组 `array` 所占用的内存是连续的，比如长度`N` 的指是 `2` 的话，那么内存中的数组元素的布局顺序是这样的：

![img](https://gitee.com/studylihai/pic-repository/raw/master/%5Cimg/20201119124227.png)
...
如果你肯定代码中的 `if` 中的表达式判断为 `true` 的概率比较高，我们可以使用==显示分支预测工具==，比如在 C/C++ 语言中编译器提供了 `likely` 和 `unlikely` 这两种宏，如果 `if` 条件为 `ture` 的概率大，则可以用 `likely` 宏把 `if` 里的表达式包裹起来，反之用 `unlikely` 宏。
...
![img](https://mmbiz.qpic.cn/mmbiz_png/J0g14CUwaZcKapyUj66hkhTng5Zof8DUvZ5zh4RImou6azJ9KMevyTDPcTHLffjYgxyckdaggftIY6hJgiarkYg/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
内存地址映射到 CPU Cache 地址里的策略有很多种，其中比较简单是直接映射 Cache，它巧妙的把内存地址拆分成「索引（index） + 组标记(tag) + 偏移量(offset)」的方式，使得我们可以将很大的内存地址，映射到很小的 CPU Cache 地址里。
...
[**天啦噜！知道硬盘很慢，但没想到比 CPU Cache 慢 10000000 倍**](https://mp.weixin.qq.com/s?__biz=MzUxODAzNDg4NQ==&mid=2247485960&idx=1&sn=476d40b3e272149ba6c7370340e9768f&scene=21#wechat_redirect)
