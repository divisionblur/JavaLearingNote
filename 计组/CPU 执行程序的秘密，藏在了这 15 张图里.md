...
![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085248.png)
...
![图来源自：http://www.kristergustafsson.me/turing-machine/](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085313.webp)图来源自：http://www.kristergustafsson.me/turing-machine/

图灵机的基本组成如下：

- 有一条「纸带」，纸带由一个个连续的格子组成，每个格子可以写入字符，纸带就好比内存，而纸带上的格子的字符就好比内存中的数据或程序；
- 有一个「读写头」，读写头可以读取纸带上任意格子的字符，也可以把字符写入到纸带的格子；
- 读写头上有一些部件，比如==存储单元、控制单元以及运算单元：==
  1、存储单元用于存放数据；
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085325.png)



- 接着，读写头读入 1 到存储设备中，这个存储设备称为图灵机的状态；

![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085329.png)



- 然后读写头向右移动一个格，用同样的方式把 2 读入到图灵机的状态，于是现在图灵机的状态中存储着两个连续的数字， 1 和 2；

![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085334.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085337.png)



- 最后，运算单元将结果返回给控制单元，控制单元将结果传输给读写头，读写头向右移动，把结果 3 写入到纸带的格子中；

![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085340.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085343.png)
...
==CPU 内部还有一些组件，常见的有寄存器、控制单元和逻辑运算单元==等。其中，控制单元负责控制 CPU 工作，逻辑运算单元负责计算，而寄存器可以分为多种类，每种寄存器的功能又不尽相同。
...
- *指令寄存器*，用来存放程序计数器指向的指令，也就是指令本身(一串二进制)，指令被执行完成之前，指令都存储在这里。
...
**为了避免低效率的串行传输的方式，线路的位宽最好一次就能访问到所有的内存地址。**CPU 要想操作的内存地址就需要地址总线，如果地址总线只有 1 条，那每次只能表示 「0 或 1」这两种情况，所以 CPU 一次只能操作 2 个内存地址，如果想要 CPU 操作 4G 的内存，那么就需要 32 条地址总线，因为 `2 ^ 32 = 4G`。那么操作16G内存需要34条地址总线
...
程序实际上是一条一条指令，所以程序的运行过程就是把每一条指令一步一步的执行起来，负责执行指令的就是 CPU 了。(指令寄存器用来存放程序计数器指向的指令)

![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085349.png)
...
针对汇编代码，我们还需要用**汇编器翻译成机器码**，这些机器码由 0 和 1 组成的机器语言，这一条条机器码，就是一条条的**计算机指令**，这个才是 CPU 能够真正认识的东西。
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085353.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085357.png)
...
![img](https://gitee.com/studylihai/pic-repository/raw/master/img/20201115085359.png)
